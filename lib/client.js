window.__ModuleLoader__.load({ id: "dsh-codex-tool-view", factory: () => {
  ...
  const TOOL_PHASES = new Map([
    ["write", "edits"], ["write_file", "edits"], ["edit", "edits"],
    ["apply_patch", "edits"], ["str_replace_editor", "edits"], ["create_file", "edits"],
    ["bash", "commands"], ["shell", "commands"], ["pwsh", "commands"],
    ["powershell", "commands"], ["exec", "commands"], ["exec_command", "commands"],
    ["run_command", "commands"], ["read", "reads"], ["read_file", "reads"],
    ["grep", "searches"], ["glob", "searches"], ["find", "searches"],
    ["search", "searches"], ["search_files", "searches"],
    ["web_search", "web"], ["web_fetch", "web"], ["open_url", "web"],
    ["task", "agents"], ["spawn_agent", "agents"], ["send_message", "agents"],
    ["wait_agent", "agents"], ["todo_write", "planning"], ["update_plan", "planning"]
  ]);
  ...
  function mount(document2) {
    ...
    const render = () => {
      ...
      for (const parent of flowParents(document2)) {
        ...
        for (const row of new Set([...finalAssistantRows, ...substantiveAssistantRows])) {
