window.__ModuleLoader__.load({ id: "dsh-codex-tool-view", factory: (require) => {
  ...
  const PHASE_META = {
    thinking: { title: "思考过程", running: "正在思考", unit: "段", short: "思考" },
    edits: { title: "编辑了文件", running: "正在编辑文件", unit: "项", short: "编辑" },
    commands: { title: "运行了命令", running: "正在运行命令", unit: "条", short: "命令" },
    reads: { title: "读取了文件", running: "正在读取文件", unit: "项", short: "读取" },
    searches: { title: "搜索了代码", running: "正在搜索代码", unit: "项", short: "搜索" },
    web: { title: "访问了网页", running: "正在访问网页", unit: "项", short: "网页" },
    agents: { title: "调用了子任务", running: "正在调用子任务", unit: "项", short: "子任务" },
    planning: { title: "更新了计划", running: "正在更新计划", unit: "项", short: "计划" },
    context: { title: "处理了上下文", running: "正在处理上下文", unit: "项", short: "上下文" },
    system: { title: "执行了系统操作", running: "正在执行系统操作", unit: "项", short: "系统" },
    tools: { title: "使用了工具", running: "正在使用工具", unit: "项", short: "工具" }
  };
  /** Most phases named in one folded-Turn summary before the tail is folded away. */
  const SUMMARY_PHASE_LIMIT = 4;
...
/* The per-phase reading of a folded Turn, appended to DSH's own disclosure row.
   Generated content keeps React's children untouched; the flex order seats it
   between the native label and the chevron. */
body [data-turn-process][data-codex-phase-summary]::after {
  content: "· " attr(data-codex-phase-summary);
  order: 1;
  flex: none;
  margin-left: 8px;
...
`;
  ...
  /**
   * Compact "读取 3 · 命令 2" reading of the phases present in the given rows.
   * A folded Turn hides its members, so per-phase headers cannot show there; this
   * string is what rides on the native disclosure row instead.
   * @param rows - the Turn's member rows.
   * @returns the summary text, or null when no phase is named.
   */
  function phaseSummary(rows) {
    const counts = new Map();
    for (const name of toolNames(rows)) {
      const phase = TOOL_PHASES.get(name.toLowerCase()) ?? "tools";
    ...
    for (const [phase, count] of counts) {
      if (index === SUMMARY_PHASE_LIMIT) {
        parts.push("…");
        break;
      }
      parts.push(`${PHASE_META[phase]?.short ?? PHASE_META.tools.short} ${count}`);
      index += 1;
    }
    return parts.join(" · ");
  }
  ...
  /**
   * Publish that summary onto each folded Turn's native disclosure row, and clear
   * it once the Turn is open again so the per-phase headers speak for themselves.
   * @param container - one transcript column.
   * @param rows - that column's direct flow rows.
   */
  function syncFoldSummaries(container, rows) {
    const hiddenByTurn = new Map();
    for (const row of rows) {
      const turn = row.dataset.chatTurn;
      if (turn === undefined || row.closest("[hidden]") === null) continue;
      const bucket = hiddenByTurn.get(turn);
      if (bucket === undefined) hiddenByTurn.set(turn, [row]);
      else bucket.push(row);
    }
    for (const button of container.querySelectorAll("[data-turn-process]")) {
      if (!(button instanceof HTMLElement)) continue;
      const summary = phaseSummary(hiddenByTurn.get(button.getAttribute("data-turn-process")) ?? []);
      if (summary === null) delete button.dataset.codexPhaseSummary;
      else button.dataset.codexPhaseSummary = summary;
    }
  }
  ...
  function mount(document2) {
    ...
    const clearDecorations = () => {
      ...
      for (const button of document2.querySelectorAll("[data-codex-phase-summary]")) {
        delete button.dataset.codexPhaseSummary;
      }
    };
    ...
    const render = () => {
      ...
      for (const parent of flowParents(document2)) {
        ...
        // A folded Turn shows its own native disclosure row: publish the phase
        // summary there, because per-phase headers cannot appear inside a fold.
        syncFoldSummaries(parent, rows);
