...
DSH 默认 compact 视图会把已结束的一轮收进原生「过程」折叠行。插件会把该轮的类型分布摘要挂到这一行上，例如 `12 个工具调用 · 读取 3 · 命令 2`，无需展开即可看出这一轮都做了什么。展开后摘要让位给分组标题。
...
较新的 DSH 默认使用 compact 对话视图：一轮结束后，整段过程被收进原生「过程」折叠行，成员行以 `hidden="until-found"` 隐藏。插件在这两种状态下都提供活动类型的信息：

- **折叠时**：把该轮的类型分布作为摘要挂到原生折叠行上（形如 `12 个工具调用 · 读取 3 · 命令 2`），用 `data-codex-phase-summary` 属性承载、由 CSS `::after` 渲染，不改动 React 管理的子节点。阶段多于 4 类时尾部折叠为 `…`。
- **展开时**：原生控件展开后，插件在过程内部按活动类型给出分组标题（「读取了文件 3 项」等），并可逐组折叠。
- **交还折叠**：用户展开某一组后又把它折叠，若该轮已无可见内容，插件会折叠原生控件本身，把折叠状态交还原生 UI。

因此分组标题只在原生折叠展开后出现——这是刻意的：折叠状态下画标题会留下空标题行。
